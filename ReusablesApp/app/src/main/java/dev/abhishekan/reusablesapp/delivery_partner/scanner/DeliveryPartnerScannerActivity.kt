...
import dev.abhishekan.reusablesapp.databinding.ActivityDeliveryPartnerScannerBinding
...
@AndroidEntryPoint
class DeliveryPartnerScannerActivity : AppCompatActivity() {

    private lateinit var binding: ActivityDeliveryPartnerScannerBinding
    private val viewModel by viewModels<DeliveryPartnerScannerViewModel>()
    private lateinit var codeScanner: CodeScanner


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityDeliveryPartnerScannerBinding.inflate(layoutInflater)
        ...
        binding.confirmReturnOrderBtn.setOnClickListener {
            ...
            if (itemUuids.isEmpty()) {
                ...
                viewModel.updateOrderToCompleted(UpdateOrderToCompletedRequest(item_uuids = itemUuids))
            }
        }

        lifecycleScope.launch {
            viewModel.updateOrderToCompletedResponse.collect {
                when (it) {
                    State.Empty -> {}
                    is State.Failed -> {
                    ...
                    is State.Success<*> -> {
                        Toast.makeText(
                            this@DeliveryPartnerScannerActivity,
                            "Pickup completed!",
                            Toast.LENGTH_LONG
                        ).show()
                        viewModel.clearScannedCodes()
